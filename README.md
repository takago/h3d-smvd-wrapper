# Hunyuan3D-2 と SyncMVD を使ったテクスチャ付き3Dメッシュ作成支援ツール
...
（Hunyuan3D-2 と SyncMVD はそれぞれ Conda 環境で用意してください。 本ツール自体はPythonの標準ライブラリのみを使用するだけなので，各Linuxディストリビューションの標準Pythonをそのまま使えばよいです。）
