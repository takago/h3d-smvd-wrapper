...
### 🔸ヒント：
- 自分の環境に合わせて書き換えてください。基本的には<USER_NAME>の変更になると思います。
- SyncMVD のスクリプトは `run_experiment.py` となっていますが適宜変更してください。画像生成時のチェックポイントやVAEを変えるなど、ファイルを改変して使っている場合はそのファイル名を指定してください）。
